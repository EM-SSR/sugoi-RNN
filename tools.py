...
def simulate(
    ...
    # --- tqdm の準備 ---
    total_iters = steps if steps else (episodes * len(envs) if episodes else None)
    pbar = tqdm(
        total=total_iters,
        desc="Simulating",
        unit="step",
    ...
    while (steps and step < steps) or (episodes and episode < episodes):
        # 環境リセット（done==True の env をまとめて）
        if done.any():
            idxs = np.where(done)[0].tolist()
            results = [envs[i].reset() for i in idxs]
            results = [r() for r in results]
            for idx, res in zip(idxs, results):
                t = {k: convert(v) for k, v in res.items()}
                t["reward"], t["discount"] = 0.0, 1.0
                add_to_cache(cache, envs[idx].id, t)
                obs[idx] = res
        ...
        steps_out = [r() for r in results]
        obs, reward, done = zip(*[o[:3] for o in steps_out])
        obs, reward = list(obs), list(reward)
        done = np.stack(done)
        ...
        # プログレスバー更新（必ず refresh=True）
        if pbar:
            pbar.update(len(envs))
            pbar.set_postfix({"step": step, "eps": episode}, refresh=True)
        if epbar and done.any():
            epbar.update(int(done.sum()))

        # キャッシュへの追加
        for a, res, env in zip(action, steps_out, envs):
            o, r, d, info = res
            trans = {k: convert(v) for k, v in o.items()}
            if isinstance(a, dict):
                trans.update(a)
            else:
                trans["action"] = a
            trans["reward"] = r
            trans["discount"] = info.get("discount", np.array(1 - float(d)))
            add_to_cache(cache, env.id, trans)
        ...
        if done.any():
            ...
            for i in done_idxs:
                ...
                ep_score = float(np.array(cache[envs[i].id]["reward"]).sum())
                video = cache[envs[i].id]["image"]
                for key in list(cache[envs[i].id].keys()):
                    if "log_" in key:
                        logger.scalar(key, float(np.sum(cache[envs[i].id][key])))
                        cache[envs[i].id].pop(key)

                if not is_eval:
                    size = erase_over_episodes(cache, limit)
                    logger.scalar("dataset_size", size)
                    logger.scalar("train_return", ep_score)
                    logger.scalar("train_length", ep_len)
                    logger.scalar("train_episodes", len(cache))
                    logger.write(step=logger.step)
                else:
                    if "eval_scores" not in locals():
                        eval_scores, eval_lengths, eval_done = [], [], False
                    eval_scores.append(ep_score)
                    eval_lengths.append(ep_len)
                    avg_score = sum(eval_scores) / len(eval_scores)
                    avg_len = sum(eval_lengths) / len(eval_lengths)
                    logger.video("eval_policy", np.array(video)[None])
                    if len(eval_scores) >= episodes and not eval_done:
                        logger.scalar("eval_return", avg_score)
                        logger.scalar("eval_length", avg_len)
...
class ContDist:
    ...
    def log_prob(self, x):
        return self._dist.log_prob(x.to('cuda'))
